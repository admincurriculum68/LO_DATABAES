...
test('groupLearningOutcomesByArea เรียงด้านตามหลักสูตร ด้านที่โรงเรียนเพิ่มต่อท้าย และรวมชื่อด้านสุขภาพเดิมกับใหม่', () => {
    const groups = groupLearningOutcomesByArea([
        lo('e1', 'ความสามารถด้านภาษาอังกฤษ', 9, 'อังกฤษ1'),
        lo('h1', 'ความสามารถด้านสุขภาพกายและจิต', 6, 'สุขภาพ2'),
        lo('r2', 'ความสามารถด้านการอ่าน', 1, 'อ่าน2'),
        lo('r1', 'ความสามารถด้านการอ่าน', 1, 'อ่าน1'),
        lo('h2', 'ความสามารถด้านสุขภาพกายและสุขภาวะจิต', 6, 'สุขภาพ1'),
        lo('x', '', null, 'ทั่วไป1'),
    ]);
    assert.deepEqual(groups.map(g => g.area), ['ความสามารถด้านการอ่าน', 'ความสามารถด้านสุขภาพกายและสุขภาวะจิต', 'ความสามารถด้านภาษาอังกฤษ', 'ไม่ระบุด้านความสามารถ']);
    assert.deepEqual(groups[0].los.map(l => l.lo_code), ['อ่าน1', 'อ่าน2']);
    assert.deepEqual(groups[1].los.map(l => l.lo_code), ['สุขภาพ1', 'สุขภาพ2']);
    assert.equal(shortAreaName('ความสามารถด้านการคิดคำนวณ'), 'การคิดคำนวณ');
});
