        ...
        <Layout title="กำหนดครูประจำรายวิชา">
            <div className="mx-auto max-w-6xl space-y-5 pb-12">
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <label className="block"><span className="mb-1.5 block text-sm font-extrabold text-slate-800">รายวิชา</span><select value={subjectId} onChange={event => setSubjectId(event.target.value)} className="min-h-12 w-full rounded-xl border border-field bg-white px-4 text-sm font-bold"><option value="">เลือกรายวิชา</option>{subjects.map(subject => <option key={subject.subject_id} value={subject.subject_id}>{subject.subject_name} · {subject.grade_level || 'ไม่ระบุชั้น'}</option>)}</select></label>
                </section>

                {loading ? <div className="h-64 animate-pulse rounded-2xl bg-slate-200" /> : rooms.length === 0 ? <section className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center"><Users className="mx-auto h-9 w-9 text-slate-500" /><h2 className="mt-3 font-extrabold text-slate-800">รายวิชานี้ยังไม่มีกลุ่มเรียน</h2><p className="mt-1 text-sm text-slate-600">จัดนักเรียนเข้ากลุ่มเรียนและระบุห้องก่อน จึงจะกำหนดขอบเขตห้องให้ครูได้</p></section> : (
                    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-extrabold text-slate-950">เลือกครูในแต่ละห้อง</h2><p className="mt-1 text-xs text-slate-500">ทำเครื่องหมายได้หลายครูต่อห้องและหลายห้องต่อครู</p></div><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-500" /><input aria-label="ค้นหาชื่อครู" value={query} onChange={event => setQuery(event.target.value)} placeholder="ค้นหาชื่อครู" className="min-h-10 rounded-xl border border-field pl-9 pr-3 text-sm" /></div></div>
