...
export default function AdminReportCompetency() {
    ...
    return (
        ...
        >
            ...
            <section className="mb-5 grid gap-3 rounded-2xl border border-line bg-white p-4 shadow-sm print:hidden md:grid-cols-[minmax(240px,1fr)_minmax(240px,1fr)_220px]">
                <label><span className="mb-1.5 block text-xs font-bold text-slate-600">ด้านความสามารถ</span><select value={selectedArea} onChange={event => setSelectedArea(event.target.value)} className="min-h-11 w-full rounded-xl border border-field bg-white px-3 text-sm font-bold text-slate-800"><option value="">ยังไม่มีข้อมูล</option>{areas.map(area => <option key={area} value={area}>{area}</option>)}</select></label>
                <label><span className="mb-1.5 block text-xs font-bold text-slate-600">ค้นหาผู้เรียน</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder="ชื่อ รหัส หรือห้องเรียน" className="min-h-11 w-full rounded-xl border border-field px-3 text-sm" /></label>
                <label><span className="mb-1.5 block text-xs font-bold text-slate-600">สถานะ</span><select value={status} onChange={event => setStatus(event.target.value)} className="min-h-11 w-full rounded-xl border border-field bg-white px-3 text-sm font-bold"><option value="all">ทุกสถานะ</option><option value="approved">รับรองแล้ว</option><option value="pending">รอรับรอง</option><option value="returned">ส่งกลับแก้ไข</option><option value="none">ยังไม่มีผล</option></select></label>
            </section>
