            ...
            <div className="mx-auto max-w-[1680px] space-y-5 pb-12">
                ...
                {showCreate && <form onSubmit={createGroup} className="rounded-2xl border border-indigo-200 bg-indigo-50 p-5">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        {['subject', 'project', 'activity'].includes(form.group_type) && <label className="sm:col-span-2"><span className="mb-1 block text-xs font-extrabold text-indigo-900">เชื่อมกับ{form.group_type === 'subject' ? 'วิชา' : 'รูปแบบการเรียนรู้'} *</span><select required value={form.source_id} onChange={event => setForm(previous => ({ ...previous, source_id: event.target.value }))} className="min-h-11 w-full rounded-xl border border-field bg-white px-3 text-sm font-bold"><option value="">-- เลือกรายการ --</option>{sourceOptions.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>}
                        <label className="sm:col-span-2"><span className="mb-1 block text-xs font-extrabold text-indigo-900">ชื่อกลุ่ม *</span><input required value={form.group_name} onChange={event => setForm(previous => ({ ...previous, group_name: event.target.value }))} placeholder="เช่น ภาษาอังกฤษ ป.3 กลุ่ม A" className="min-h-11 w-full rounded-xl border border-field px-3 text-sm font-bold" /></label>
                        <label><span className="mb-1 block text-xs font-extrabold text-indigo-900">ชื่อห้อง/กลุ่มที่ครูเห็น {form.group_type === 'subject' && '*'}</span><input required={form.group_type === 'subject'} value={form.room_name} onChange={event => setForm(previous => ({ ...previous, room_name: event.target.value }))} placeholder="เช่น ป.3/1 หรือ กลุ่ม A" className="min-h-11 w-full rounded-xl border border-field px-3 text-sm font-bold" /></label>
