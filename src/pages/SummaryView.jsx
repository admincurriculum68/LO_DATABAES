        ...
        <Layout title="สรุปผลรายวิชา">
            ...
            <div className="mx-auto w-full max-w-[1800px]">
                <header className="summary-controls mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div><button onClick={() => navigate('/')} className="mb-2 inline-flex min-h-11 items-center gap-2 rounded-lg px-2 text-sm font-bold text-indigo-700 hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-indigo-500"><ArrowLeft className="h-4 w-4" /> กลับไปงานของฉัน</button><div className="flex items-center gap-2 text-sm font-bold text-indigo-700"><FileBarChart2 className="h-4 w-4" /> รายงานผลรายวิชา</div><h1 className="mt-1 text-2xl font-extrabold text-slate-950">{subject?.subject_name || 'กำลังโหลดรายวิชา'}</h1><p className="mt-1 text-sm text-slate-600">ตารางที่ 1 · ผลลัพธ์การเรียนรู้ระดับรายวิชา · ชั้น {subject?.grade_level || '-'} · ภาคเรียนที่ {subject?.semester || '-'}/{subject?.academic_year || '-'}</p></div>
                ...
                {loadError ? (
                    <section className="rounded-2xl border border-rose-200 bg-rose-50 p-6" role="alert"><div className="flex gap-3"><AlertCircle className="mt-0.5 h-6 w-6 shrink-0 text-rose-700" /><div><h2 className="font-extrabold text-rose-950">เปิดรายงานไม่สำเร็จ</h2><p className="mt-1 text-sm text-rose-800">{loadError}</p></div></div></section>
                ) : loading ? (
