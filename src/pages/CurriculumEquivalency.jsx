        ...
        <Layout title="เทียบผลการเรียน 2568 ไป 2551">
            <div className="mx-auto max-w-7xl space-y-5 pb-12">
                ...
                <div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
                    <aside className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-4"><label className="relative block"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-500" /><input aria-label="ค้นหานักเรียนด้วยรหัส ชื่อ หรือห้อง" value={query} onChange={event => setQuery(event.target.value)} placeholder="ค้นหารหัส ชื่อ หรือห้อง" className="min-h-11 w-full rounded-xl border border-field pl-9 pr-3 text-sm" /></label></div><div className="max-h-[650px] divide-y divide-slate-100 overflow-y-auto">{visibleStudents.map(student => <button key={student.student_id} onClick={() => setStudentId(student.student_id)} className={`w-full p-4 text-left ${studentId === student.student_id ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}><p className="font-extrabold text-slate-900">{student.prefix || ''}{student.first_name} {student.last_name}</p><p className="mt-1 text-xs text-slate-600">{student.student_code || '-'} · {student.current_room || 'ไม่ระบุห้อง'}</p></button>)}</div></aside>
                    <section className="space-y-5">
                        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-extrabold text-slate-950">{selectedStudent ? `${selectedStudent.prefix || ''}${selectedStudent.first_name} ${selectedStudent.last_name}` : 'เลือกนักเรียน'}</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-extrabold text-slate-700">ด้านความสามารถตามหลักสูตร 2568 *</span><input value={form.source_competency_area} onChange={event => updateForm('source_competency_area', event.target.value)} className="min-h-11 w-full rounded-xl border border-field px-3 text-sm" placeholder="เช่น ความสามารถด้านการคิดคำนวณ" /></label><label><span className="mb-1.5 block text-sm font-extrabold text-slate-700">กลุ่มสาระการเรียนรู้ตามหลักสูตร 2551 *</span><input value={form.target_learning_area} onChange={event => updateForm('target_learning_area', event.target.value)} className="min-h-11 w-full rounded-xl border border-field px-3 text-sm" placeholder="เช่น คณิตศาสตร์" /></label><label><span className="mb-1.5 block text-sm font-extrabold text-slate-700">รายวิชาปลายทาง</span><input value={form.target_subject_name} onChange={event => updateForm('target_subject_name', event.target.value)} className="min-h-11 w-full rounded-xl border border-field px-3 text-sm" placeholder="ถ้ามี" /></label><label><span className="mb-1.5 block text-sm font-extrabold text-slate-700">ผลที่เทียบได้</span><input value={form.target_result} onChange={event => updateForm('target_result', event.target.value)} className="min-h-11 w-full rounded-xl border border-field px-3 text-sm" placeholder="เช่น ผ่าน หรือระดับผลการเรียน" /></label><label className="sm:col-span-2"><span className="mb-1.5 block text-sm font-extrabold text-slate-700">หลักฐานจากหลักสูตร 2568 *</span><textarea rows="4" value={form.source_evidence} onChange={event => updateForm('source_evidence', event.target.value)} className="w-full rounded-xl border border-field p-3 text-sm leading-6" placeholder="สรุป LO ข้อความพฤติกรรม ชิ้นงาน หรือหลักฐานที่ใช้เทียบ" /></label><label className="sm:col-span-2"><span className="mb-1.5 block text-sm font-extrabold text-slate-700">เหตุผลการเทียบผล</span><textarea rows="3" value={form.decision_reason} onChange={event => updateForm('decision_reason', event.target.value)} className="w-full rounded-xl border border-field p-3 text-sm leading-6" placeholder="อธิบายความสอดคล้องของเนื้อหา เวลาเรียน และหลักฐาน" /></label></div><div className="mt-5 flex flex-wrap justify-end gap-2"><button onClick={() => save('draft')} disabled={saving} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-5 text-sm font-extrabold text-slate-700"><Save className="h-4 w-4" />บันทึกฉบับร่าง</button><button onClick={() => save('approved')} disabled={saving} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-indigo-700 px-5 text-sm font-extrabold text-white"><CheckCircle2 className="h-4 w-4" />รับรองผลเทียบ</button></div></section>
