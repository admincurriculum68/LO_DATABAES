        ...
        <Layout title="เทียบผลการเรียน 2568 ไป 2551">
            <div className="mx-auto max-w-7xl space-y-5 pb-12">
                ...
                <div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
                    <aside className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-4"><label className="relative block"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-500" /><input aria-label="ค้นหานักเรียนด้วยรหัส ชื่อ หรือห้อง" value={query} onChange={event => setQuery(event.target.value)} placeholder="ค้นหารหัส ชื่อ หรือห้อง" className="min-h-10 w-full rounded-xl border border-field pl-9 pr-3 text-sm" /></label></div><div className="max-h-[650px] divide-y divide-slate-100 overflow-y-auto">{visibleStudents.map(student => <button key={student.student_id} onClick={() => setStudentId(student.student_id)} className={`w-full p-4 text-left ${studentId === student.student_id ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}><p className="font-extrabold text-slate-900">{student.prefix || ''}{student.first_name} {student.last_name}</p><p className="mt-1 text-xs text-slate-600">{student.student_code || '-'} · {student.current_room || 'ไม่ระบุห้อง'}</p></button>)}</div></aside>
