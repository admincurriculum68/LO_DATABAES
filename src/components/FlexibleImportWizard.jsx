...
import { citizenIdFormatError, isCitizenIdFormat, normalizeThaiDob, sanitizeCitizenId } from '../lib/importSanitizers';
...
const validateRecord = (record, fields) => {
    ...
    fields.forEach(field => {
        ...
        if (field.kind === 'citizen' && value && !isCitizenIdFormat(value)) errors.push(`${field.label}: ${citizenIdFormatError(value)}`);
...
export default function FlexibleImportWizard({ initialType = 'students', onCancel, onConfirm }) {
    ...
    return (
        <section className="overflow-hidden rounded-2xl border border-line bg-white" aria-label="ตัวช่วยนำเข้าข้อมูล">
            <header className="border-b border-line px-5 py-4 sm:px-6">
                <div className="flex items-start justify-between gap-4">
                    <div><h2 className="text-lg font-bold text-slate-950">ตัวช่วยนำเข้าข้อมูล</h2><p className="mt-1 text-sm text-slate-600">ไม่จำเป็นต้องเปลี่ยนชื่อคอลัมน์ในไฟล์ ระบบช่วยจับคู่ให้และตรวจสอบก่อนบันทึก</p></div>
            ...
            {step === 1 && <div className="space-y-5 p-5 sm:p-6">
                <div><label htmlFor="import-type" className="mb-1.5 block text-sm font-bold text-slate-800">ต้องการเพิ่มข้อมูลอะไร?</label><select id="import-type" value={importType} onChange={event => setImportType(event.target.value)} className="min-h-12 w-full rounded-xl border border-field bg-white px-4 text-sm font-bold">{Object.entries(IMPORT_SCHEMAS).map(([key, item]) => <option key={key} value={key}>{item.title}</option>)}</select><p className="mt-2 text-sm text-slate-600">{schema.description}</p></div>
            ...
            {step === 2 && <div className="space-y-5 p-5 sm:p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-bold text-slate-700">ชีตข้อมูล</span><select value={sheetIndex} onChange={event => changeSheet(Number(event.target.value))} className="min-h-11 w-full rounded-xl border border-field bg-white px-3 text-sm font-bold">{sheets.map((sheet, index) => <option key={sheet.name} value={index}>{sheet.name} · {sheet.rows.length} แถว</option>)}</select></label><label><span className="mb-1.5 block text-sm font-bold text-slate-700">แถวที่เป็นชื่อคอลัมน์</span><select value={headerRow} onChange={event => changeHeaderRow(event.target.value)} className="min-h-11 w-full rounded-xl border border-field bg-white px-3 text-sm font-bold">{rows.slice(0, 20).map((row, index) => <option key={index} value={index}>แถวที่ {index + 1}: {(row || []).filter(Boolean).slice(0, 4).join(' · ') || 'ไม่มีข้อมูล'}</option>)}</select></label></div>
                <div className="overflow-hidden rounded-xl border border-line"><div className="border-b border-line bg-slate-50 px-4 py-3"><h3 className="font-bold text-slate-900">จับคู่ข้อมูลที่ระบบต้องการกับคอลัมน์ในไฟล์</h3><p className="mt-1 text-xs text-slate-600">ระบบจับคู่ให้อัตโนมัติแล้ว โปรดตรวจช่องที่มีเครื่องหมาย *</p></div><div className="grid gap-px bg-slate-200 sm:grid-cols-2">{schema.fields.map(field => <label key={field.key} className="bg-white p-4"><span className="mb-1.5 flex items-center justify-between text-sm font-bold text-slate-800"><span>{field.label}{field.required && <span className="text-rose-600"> *</span>}</span>{mapping[field.key] !== '' && <CheckCircle2 className="h-4 w-4 text-emerald-700" />}</span><select value={mapping[field.key] ?? ''} onChange={event => setMapping(previous => ({ ...previous, [field.key]: event.target.value }))} className={`min-h-11 w-full rounded-xl border bg-white px-3 text-sm font-bold ${field.required && (mapping[field.key] === '' || mapping[field.key] === undefined) ? 'border-rose-400 text-rose-800' : 'border-field text-slate-800'}`}><option value="">— ไม่มีคอลัมน์นี้ —</option>{headers.map((header, index) => String(header ?? '').trim() && <option key={`${index}-${header}`} value={String(index)}>{String(header)} · คอลัมน์ {index + 1}</option>)}</select></label>)}</div></div>
                ...
                <div className="flex justify-between gap-3"><button onClick={() => setStep(1)} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold text-slate-700"><ArrowLeft className="h-4 w-4" />ย้อนกลับ</button><button onClick={() => setStep(3)} disabled={requiredMissing.length > 0} className="action-primary inline-flex min-h-11 items-center gap-2 rounded-xl px-5 text-sm font-bold disabled:opacity-40">ตรวจสอบข้อมูล<ArrowRight className="h-4 w-4" /></button></div>
            </div>}

            {step === 3 && <div className="space-y-5 p-5 sm:p-6">
                <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-line bg-slate-50 p-4"><p className="text-xs font-bold text-slate-600">ข้อมูลทั้งหมด</p><p className="mt-1 text-2xl font-bold text-slate-950">{mappedRows.length.toLocaleString()} <span className="text-sm">รายการ</span></p></div><div className="surface-success rounded-xl border border-emerald-200 p-4"><p className="text-xs font-bold text-emerald-800">พร้อมนำเข้า</p><p className="mt-1 text-2xl font-bold text-emerald-900">{readyRows.length.toLocaleString()} <span className="text-sm">รายการ</span></p></div><div className={`rounded-xl border p-4 ${errorCount ? 'surface-danger border-rose-200' : 'border-line bg-white'}`}><p className={`text-xs font-bold ${errorCount ? 'text-rose-800' : 'text-slate-600'}`}>ต้องแก้ไข</p><p className={`mt-1 text-2xl font-bold ${errorCount ? 'text-rose-900' : 'text-slate-800'}`}>{errorCount.toLocaleString()} <span className="text-sm">รายการ</span></p></div></div>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-bold text-slate-950">ตรวจสอบและแก้ไขข้อมูลทุกแถว</h3><p className="mt-1 text-xs text-slate-600">แสดงครั้งละ 50 แถว เลื่อนไปหน้าอื่นเพื่อแก้รายการที่เหลือได้ ระบบจะนำเข้าเฉพาะรายการที่ไม่มีข้อผิดพลาด</p></div>{errorCount > 0 && <button onClick={() => { setShowOnlyErrors(value => !value); setPreviewPage(1); }} className={`min-h-11 rounded-xl border px-3 text-sm font-bold ${showOnlyErrors ? 'action-danger border-rose-700' : 'border-rose-300 bg-white text-rose-800'}`}>{showOnlyErrors ? 'แสดงทุกรายการ' : `แสดงเฉพาะ ${errorCount} รายการที่ผิด`}</button>}</div>
                <div className="max-h-[520px] overflow-auto rounded-xl border border-line"><table className="min-w-max w-full text-sm"><thead className="sticky top-0 z-10 bg-slate-100 text-slate-700"><tr><th className="px-3 py-3 text-center">แถว</th><th className="px-3 py-3 text-left">สถานะ</th>{schema.fields.map(field => <th key={field.key} className="min-w-[160px] px-3 py-3 text-left">{field.label}</th>)}</tr></thead><tbody className="divide-y divide-line bg-white">{visiblePreview.map(item => <tr key={item.sourceRow} className={item.errors.length ? 'surface-danger' : ''}><td className="px-3 py-2 text-center font-mono text-xs text-slate-600">{item.sourceRow}</td><td className="max-w-[220px] px-3 py-2">{item.errors.length ? <span className="text-xs font-bold leading-5 text-rose-700">{item.errors.join(' · ')}</span> : <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700"><CheckCircle2 className="h-4 w-4" />พร้อม</span>}</td>{schema.fields.map(field => { const editKey = `${item.sourceRow}:${field.key}`; return <td key={field.key} className="px-2 py-2"><input aria-label={`${field.label} แถวที่ ${item.sourceRow}`} value={item.record[field.key] ?? ''} onChange={event => setEdits(previous => ({ ...previous, [editKey]: event.target.value }))} onBlur={event => setEdits(previous => ({ ...previous, [editKey]: transformValue(event.target.value, field) }))} className={`min-h-11 w-full rounded-lg border px-2 text-xs ${item.errors.length && (!item.record[field.key] || (field.kind === 'citizen' && !isCitizenIdFormat(item.record[field.key]))) ? 'border-rose-400 bg-white' : 'border-field bg-white'}`} /></td>; })}</tr>)}</tbody></table></div>
                {previewPageCount > 1 && <div className="flex items-center justify-between rounded-xl border border-line bg-slate-50 p-3"><span className="text-sm font-bold text-slate-600">หน้า {previewPage} จาก {previewPageCount} · {previewRows.length.toLocaleString()} รายการ</span><div className="flex gap-2"><button type="button" onClick={() => setPreviewPage(value => Math.max(1, value - 1))} disabled={previewPage === 1} className="min-h-11 rounded-lg border border-slate-300 bg-white px-4 text-sm font-bold disabled:opacity-40">ก่อนหน้า</button><button type="button" onClick={() => setPreviewPage(value => Math.min(previewPageCount, value + 1))} disabled={previewPage === previewPageCount} className="action-primary min-h-11 rounded-lg px-4 text-sm font-bold disabled:opacity-40">ถัดไป</button></div></div>}
                {canAutoEnroll && <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-line bg-slate-50 p-4"><input type="checkbox" checked={autoEnroll} onChange={event => setAutoEnroll(event.target.checked)} className="mt-0.5 h-5 w-5 shrink-0 accent-indigo-700" /><span><span className="block text-sm font-bold text-slate-900">จัดนักเรียนทั้งห้องเข้าวิชาให้อัตโนมัติ</span><span className="mt-0.5 block text-xs leading-5 text-slate-600">นักเรียนทุกคนในห้องที่ระบุในไฟล์จะถูกเพิ่มเข้าวิชานั้น ไม่ต้องกดเพิ่มทีละห้อง ต้องนำเข้านักเรียนก่อน ถ้าบางวิชามีนักเรียนเรียนไม่ครบทั้งห้อง ให้ปิดตัวเลือกนี้ หรือนำนักเรียนออกภายหลังในเมนูจัดนักเรียนเข้ารายวิชา</span></span></label>}
                <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><button onClick={() => setStep(2)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold text-slate-700"><ArrowLeft className="h-4 w-4" />กลับไปจับคู่คอลัมน์</button><button onClick={confirm} disabled={!readyRows.length || loading} className="action-success inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-6 text-sm font-bold disabled:opacity-40"><Upload className="h-4 w-4" />{loading ? 'กำลังนำเข้า...' : `ยืนยันนำเข้า ${readyRows.length.toLocaleString()} รายการ`}</button></div>
            </div>}
